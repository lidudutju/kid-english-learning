...
**标题改名：** 播放页标题右侧的「改名」可随时修改标题，上传前的输入框改的是同一个字段。iPhone 交出的文件名常是一串十六进制（`dd2d35a7f664…`），YouTube 标题则取决于上传者。标题是全库唯一可手动编辑的字段：其余信息要么由字节推导（时长、分辨率、哈希），要么属于孩子的观看记录，可编辑即意味着可失真。
