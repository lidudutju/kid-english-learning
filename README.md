...
| `apps/agent` | 家里的 Mac | yt-dlp 下载（含字幕）→ ffmpeg 转码 → 数重点词 → 上传 R2 |
| `packages/shared` | 三边共用 | Zod 契约、链接解析、任务状态、学习规则、SHA-256、字幕解析、重点词 |

先读 [`CONTEXT.md`](./CONTEXT.md)（术语表），再读 [`docs/adr/`](./docs/adr)（五个不好回头的决定）。
这两份文件解释了下面每一步**为什么**长这样。

> 现在能用的：登录 → 加视频（粘链接 / 传文件）→ 下载归一化上传 → 列表与搜索（含歌词）→
> iPhone 播放（跟随高亮的字幕 + 这个视频在教哪几个词）→ 记录看了几遍、学到哪、喜不喜欢 →
> 每天给一份「今天要看」。含去重硬拦、任务进度、自动重试。
> 还没有：AI。手机传上来的视频没有字幕，这是故意的
> （[ADR-0005](./docs/adr/0005-transcripts-come-from-youtube-and-focus-words-are-counted.md)）。
...
**播放页下面有字幕和「在教这几个词」。** 字幕跟着播放走，当前那句是高亮的，点任意一句就跳过去
重放——3 岁孩子听不清的那一句，用不着拖进度条找。手指一碰字幕就停止自动跟随（不然它会跟你抢滚动条），
再点一句就恢复。上面那排黄色的词是这个视频重复得最多的词和句型，带次数：这就是它到底在教什么，
不是标签，是数出来的（[ADR-0005](./docs/adr/0005-transcripts-come-from-youtube-and-focus-words-are-counted.md)）。

**搜索也搜歌词。** 列表页那个框现在搜标题、频道、和重点词；只在这些都没命中的时候，才去服务端
搜整份字幕，结果排在下面「字幕里也提到了」。所以记不住片名、只记得「那个有 twinkle 的」也能找到。

**「今天要看」挑新视频会挑他听得懂的那个。** 同样是没看过的，重点词跟他已经学会的视频重合得多的
排前面——今晚能跟着唱的比从零开始的有用。手机传上来的视频没有字幕、也就没有重点词，
所以它们不会因为这条被挑中（但照样能播、照样记进度）。
...
pnpm check:focus            # 字幕解析、重点词统计（改了 vtt.ts 或 focus.ts 就跑）
pnpm -F @kel/worker tail    # 看线上日志
```

那三个 `check:` 不是单元测试，是对账：它们各盯着一件「错了不会报错、只会静悄悄给出错答案」的事——
一个哈希算错就会放进重复视频或拦住新视频，一个日期算错就会让复习提前一天或永远不出现，
一个字幕解析错了视频照样能播、面板照样有字，只是重点词是错的。

`check:focus` 还多担一件事：重点词是 Agent 算的、Worker 原样存的（免费版 Workers 只有 10ms CPU，
一份满长度字幕光数词就要 6ms），所以一台没更新的 Agent 会安静地写进旧算法的结果。
这个脚本是两边共同认账的那份定义。
...
**「这个视频没有字幕」** —— 两种情况。手机传上来的视频**一定**没有，因为本地语音识别被明确否决了：
儿歌 + 房间噪音 + 大人跟着唱是识别最差的场景，而重点词是会被拿去挑「今天要看」的，
猜错了不只是面板难看，是把今晚的安排指向了视频里根本没说过的词
（[ADR-0005](./docs/adr/0005-transcripts-come-from-youtube-and-focus-words-are-counted.md)）。
YouTube 视频没有字幕则是频道自己没写、自动字幕也没生成——重新添加一次也不会有，不用试。

**字幕像是断句怪、或者少一句** —— 大概是自动字幕。它是滚动的：每一句都会把屏幕上还在的那句
重说一遍，解析时要把重复的前缀剥掉，而「字幕被重画」和「这句被唱了两遍」长得一模一样。
判据是时长（重画那一下只有几十毫秒），拿不准的时候一律算「真的唱了两遍」——重复本身就是重点词的
全部依据，宁可面板里多一行。字幕那一栏右上角写着是哪种——「官方字幕」没有这个问题，「自动识别」有。

**重点词里出现了奇怪的词** —— 它只会数重复，不会理解。一个视频里反复喊的语气词就是会上榜。
停用词表在 `packages/shared/src/focus.ts` 里，是故意留短的：疑问词（what / where / how）
没有被停掉，因为「where is my nose」正是这个 app 想指出来的东西。改完跑 `pnpm check:focus`。
